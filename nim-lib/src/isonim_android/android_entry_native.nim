## Android JNI entry point for native controls variant.
##
## Identical to android_entry.nim except:
##   - imports task_app (which selects native_controls via -d:nativeControls)
##   - calls renderApp instead of renderTaskApp
##
## The JNI function names are identical because this .so is loaded by a
## separate APK flavor — only one libisonim.so is loaded per process.
##
## Compile with: -d:commandBuffer -d:nativeControls --app:lib --noMain
...
proc buildUI() =
  ...
  # Wrap in createRoot since control modules use createRenderEffect.
  # For command-buffer rendering, the root is disposed immediately after
  # the tree is captured — reactive effects don't persist across rebuilds.
  createRoot proc(dispose: proc()) =
    let store = createTaskStore()
    # Sync plain state into signal-based store
    if appState != nil:
      for t in appState.tasks:
        store.addTask(t.text)
        if t.completed:
          store.toggleTask(store.tasks.val[^1].id)
      store.setFilter(appState.filter)
    discard renderApp[AndroidRenderer, AndroidElement](
      AndroidRenderer(), store)
    dispose()
